...
directly — no transcript in the middle. It wins on **latency** (one hop) and
...
| "this dive is an offline simulator" note | You set `PROVIDER` to something other than `mock`. That's fine — there's only a mock here; the note is just letting you know. |
